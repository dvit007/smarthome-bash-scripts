...
:global Edelweiss "v1.3 08.07.2022";
...
#Параметры:
#srcAdr  - ip адрес устройства (type=ip/str)
#toAdr   - ip адрес сервера (type=ip/str)
#Comment - комментарий создаваемого правила (type=str)
#Возвращаемое значение:
#нет
#Пример вызова:
#$adNat srcAdr=$ip toAdr="192.168.43.9" Comment="Comment";
:global adNat do={
	:local srcIp [:toip $srcAdr];
	:local toIp  [:toip $toAdr];

	#Проверим есть ли уже аналогичное правило, чтобы не дублировать его
	:local addRule true;
	:foreach i in=[/ip firewall nat find src-address=$srcAdr comment=$Comment] do {
		:set addRule false;	
	}
	:if ($addRule=true) do={
		/ip firewall nat add chain=srcnat action=src-nat src-address=$srcIp \
		 to-addresses=$toIp comment=$Comment place-before=1 
	} 
}

#Удалить правило из таблицы Nat
#Параметры:
#srcAdr  - ip адрес устройства (type=ip/str)
#Comment - комментарий создаваемого правила (type=str)
#Возвращаемое значение:
#нет
#Пример вызова:
#$deNat srcAdr=$ip Comment="Comment";
:global deNat do={
	/ip firewall nat remove [ find src-address=$srcAdr comment=$Comment]
}

#Получить mac адрес по ip адресу
#Параметры:
#ipAdr - строка с ip адресом 
#Возвращаемое значение:
#macAdr - строка с mac адресом
#Пример вызова:
#:local ip [$geMac ipAdr="192.168.1.26"];
:global geMac do={
	:local macAdr;
	:foreach i in=[/ip arp find where address~$ipAdr] do={
		:set macAdr [/ip arp get $i mac-address];
	}
	return $macAdr;
}


#Получить ip адрес по mac адресу
#Параметры:
#macAdr - строка с mac адресом 
#Возвращаемое значение:
#ipAdr - type ip 
#Прмиер вызова:
#:local mac [$geIp macAdr="ХХ.ХХ.ХХ.ХХ];
:global geIp do={
	:local ipAdr;
	:foreach i in=[/ip arp find where mac-address~$macAdr] do={
		:set ipAdr [/ip arp get $i address];
	}
	return $ipAdr;
}

#Проверяет подключения локального устройства checkAdres к серверам checkServers
#Параметры:
#checkServers - массив строк с именами серверов
#checkAdres   - строка с адресом 
#Пример вызова:
#if ([$chCon checkServers={"netflix";"amazonaws"} checkAdres="192.168.1.14" ]=true) do={}
:global chCon do={
	:local Connected false;
	:do {
		:foreach i in=[/ip firewall connection find src-address~$checkAdres protocol~"tcp" tcp-state~"established"] do={
			:local srcaddress [/ip firewall connection get $i src-address];
			:local dstaddress [/ip firewall connection get $i dst-address];
			:local dstip      [:pick $dstaddress 0 [:find $dstaddress ":"]];
			
			:log info ("ip firewall connection: $srcaddress -> $dstaddress | dst ip->$dstip");
			
			:foreach i in=[/ip dns cache all find where (data~$dstip) && (type="A")] do={
				:local cacheName [/ip dns cache get $i name];
				#:log info ("|_dst name->$cacheName");        
				#:log info ("Connections to ip->$dstip | name->$cacheName");
				:foreach i in=$checkServers do={
					:if ([:find $cacheName $i] > 0) do={
						:log info ("detected connection to ip->$dstip | name->$cacheName");
						:put "Detected connection $checkAdres->$i";
						:set Connected true;
						#https://forum.mikrotik.com/viewtopic.php?t=42113
						#This is generating a run-time error for break:
						/break;
					}   
				}           
			}  
		}
		:put "Connection $checkAdres state=[$Connected]";
	} on-error={
		#:put "Connection $checkAdres->$i state=[$Conected]"; 
	}    
	return $Connected;
}
